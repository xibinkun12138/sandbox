...
import black.android.app.BRActivityThreadR;
...
import com.hello.sandbox.core.CrashHandler;
...
import com.hello.sandbox.core.env.VirtualRuntime;
...
import com.hello.sandbox.utils.compat.StrictModeCompat;
import java.io.File;
...
public class BActivityThread extends IBActivityThread.Stub {
  ...
  private final Handler mH = SandBoxCore.get().getHandler();
  ...
  public Service createService(ServiceInfo serviceInfo, IBinder token) {
    ...
    try {
      ...
      BRService.get(service)
          .attach(
              context,
              SandBoxCore.mainThread(),
              serviceInfo.name,
              token,
              mInitialApplication,
      ...
      throw new RuntimeException(
          "Unable to create service " + serviceInfo.name + ": " + e.toString(), e);
    }
  }
  ...
  public JobService createJobService(ServiceInfo serviceInfo) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Unable to create JobService " + serviceInfo.name + ": " + e.toString(), e);
    }
  }
  ...
  public synchronized void handleBindApplication(String packageName, String processName) {
    ...
    try {
      ...
      if (TextUtils.equals(packageName, "com.p1.mobile.putong")) {
        fixTantanYidun(mInitialApplication, bindData.providers);
      }
      ...
      if (TextUtils.equals(packageName, "com.immomo.momo")) {
        ...
        mInitialApplication.registerActivityLifecycleCallbacks(
            new AppLifecycleCallback() {
              @Override
  ...
  private void fixAiLiaoPhoto(Application application) throws Throwable {
    if (application.getPackageName().equals("com.mosheng")) {
      ...
      Method parsePathStrategyMethod =
          fileProviderClass.getDeclaredMethod("getPathStrategy", Context.class, String.class);
      parsePathStrategyMethod.setAccessible(true);
      Object pathStrategy =
          parsePathStrategyMethod.invoke(null, application, "com.mosheng.provider");
      Field fieldAuthority = pathStrategy.getClass().getDeclaredField("mAuthority");
      fieldAuthority.setAccessible(true);
      fieldAuthority.set(pathStrategy, "files.com.hello.miheapp");
    }
  }

  private static void fixTantanYidun(Application application, List<ProviderInfo> provider) {
  ...
  public static Context createPackageContext(ApplicationInfo info) {
    try {
      return SandBoxCore.getContext()
          .createPackageContext(
              info.packageName, Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
    } catch (Exception e) {
      e.printStackTrace();
    }
    return null;
  ...
  public IBinder getActivityThread() {
    return BRActivityThread.get(SandBoxCore.mainThread()).getApplicationThread();
  }
  ...
  public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
    if (!isInit()) {
      bindApplication(
          BActivityThread.getAppConfig().packageName, BActivityThread.getAppConfig().processName);
    }
    String[] split = providerInfo.authority.split(";");
    for (String auth : split) {
      ContentProviderClient contentProviderClient =
          SandBoxCore.getContext().getContentResolver().acquireContentProviderClient(auth);
    ...
    mH.post(
        () -> {
          Map<IBinder, Object> activities =
              BRActivityThread.get(SandBoxCore.mainThread()).mActivities();
    ...
    mH.post(
        () -> {
          ...
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
            newIntent = BRReferrerIntent.get()._new(intent, SandBoxCore.getHostPkg());
          } else {
            newIntent = intent;
          }
          Object mainThread = SandBoxCore.mainThread();
          if (BRActivityThread.get(SandBoxCore.mainThread())._check_performNewIntents(null, null)
              != null) {
            BRActivityThread.get(mainThread)
                .performNewIntents(token, Collections.singletonList(newIntent));
          } else if (BRActivityThreadNMR1.get(mainThread)
                  ._check_performNewIntents(null, null, false)
              != null) {
            BRActivityThreadNMR1.get(mainThread)
                .performNewIntents(token, Collections.singletonList(newIntent), true);
          ...
          } else if (BRActivityThreadR.get(mainThread)._check_handleNewIntent(null, null) != null) {
            BRActivityThreadR.get(mainThread)
                .handleNewIntent(
                    (ActivityClientRecord)
                        BRActivityThread.get(SandBoxCore.mainThread()).getActivityClient(token),
  ...
  private void onBeforeCreateApplication(String packageName, String processName, Context context) {
    for (AppLifecycleCallback appLifecycleCallback : SandBoxCore.get().getAppLifecycleCallbacks()) {
      appLifecycleCallback.beforeCreateApplication(
          packageName, processName, context, BActivityThread.getUserId());
    }
  }

  private void onBeforeApplicationOnCreate(
      String packageName, String processName, Application application) {
    for (AppLifecycleCallback appLifecycleCallback : SandBoxCore.get().getAppLifecycleCallbacks()) {
      appLifecycleCallback.beforeApplicationOnCreate(
          packageName, processName, application, BActivityThread.getUserId());
    }
  }

  private void onAfterApplicationOnCreate(
      String packageName, String processName, Application application) {
    for (AppLifecycleCallback appLifecycleCallback : SandBoxCore.get().getAppLifecycleCallbacks()) {
      appLifecycleCallback.afterApplicationOnCreate(
          packageName, processName, application, BActivityThread.getUserId());
    }
  }
