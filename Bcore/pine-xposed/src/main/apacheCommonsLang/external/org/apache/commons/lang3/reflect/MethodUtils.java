...
/**
 * Utility reflection methods focused on methods, originally from Commons BeanUtils. Differences
 * from the BeanUtils version may be noted, especially where similar functionality already existed
 * within Lang.
 *
 ...
 *
 * <p>There is an issue when invoking public methods contained in a default access superclass on
 * JREs prior to 1.4. Reflection locates these methods fine and correctly assigns them as public.
 * However, an <code>IllegalAccessException</code> is thrown if the method is invoked.
 *
 * <p><code>MethodUtils</code> contains a workaround for this situation. It will attempt to call
 * <code>setAccessible</code> on this method. If this call succeeds, then the method can be invoked
 * as normal. This call will only succeed when the application has sufficient security privileges.
...
public class MethodUtils {
  ...
  /**
   * MethodUtils instances should NOT be constructed in standard programming. Instead, the class
   * should be used as <code>MethodUtils.getAccessibleMethod(method)</code>.
   *
   ...
   * classes. So, for example, a <code>Boolean</code> object would match a <code>boolean</code>
   * primitive.
   *
   ...
   *
   * <p>This uses reflection to invoke the method obtained from a call to <code>
   * getAccessibleMethod()</code>.
   *
   * @param object invoke method on this object
   * @param methodName get method with this name
   * @param args use these arguments - treat null as empty array
  ...
  public static Object invokeExactStaticMethod(
      Class<?> cls, String methodName, Object[] args, Class<?>[] parameterTypes)
      throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
    if (args == null) {
      args = ArrayUtils.EMPTY_OBJECT_ARRAY;
    }
   ...
   * primitive.
   *
   * <p>This is a convenient wrapper for {@link #invokeStaticMethod(Class objectClass,String
   ...
   *
   * <p>There isn't any good reason why this method must be private. It is because there doesn't
   * seem any reason why other classes should call this rather than the higher level methods.
   *
